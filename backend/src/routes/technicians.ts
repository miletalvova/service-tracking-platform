...
router.patch("/:id/status", isAuth, isTechnician, async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (!statusId) {
            return res.status(400).json({ status: "error", statusCode: 400, message: "statusId is required" });
        }

        const updatedRequest = await TechnicianService.updateStatus(serviceRequestId, statusId, technicianId);

        if (statusId === StatusEnum.Completed || statusId === StatusEnum.Cancelled) {
            await jobAssignmentService.unassign(serviceRequestId);
        }

        return res.status(200).json({ status: "success", statusCode: 200, message: "Status updated successfully", data: updatedRequest });
    } catch (err) {
        return next(err);
    }
});
...
router.patch("/profile", isAuth, isTechnician, async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        return res.status(200).json({ status: "success", statusCode: 200, message: "Profile updated successfully", data: profile });
    } catch (err) {
        return next(err);
    }
});
